...
<div class="row justify-content-center">
    <div class="col-8">
        ...
        " method="post" class="bg-white rounded-3 shadow p-5 mb-5" enctype="multipart/form-data">
        @csrf
        @method('PATCH')

        <h2 class="h3 text-secondary fw-light mb-3">Update Profile</h2>
        <div class="row mb-3">
            <div class="col-4">
                @if(Auth::user()->avatar)
        ...
        @error('name')
        <p class="mb-9 text-danger small">{{$message}}</p>
        @enderror
        ...
        <label for="intro" class="form-label fw-bold mt-2">Introduction</label>
    ...
    {{-- UPDATE PASSWORD--}}
    <form action="{{ route('profile.update-password')}}" class="bg-white rounded-3 shadow p-5" method="post">
        @csrf
        @method('PATCH')
        
        @if(session('success_message'))
        <h5 class="mb-3 text-success">{{ session('success_message')}}</h5>
        @endif
        <h2 class="h3 fw-light text-secondary mb-3">Update Password</h2>
        ...
        <p class="mb-0 text-danger small">{{ session('incorrect_password_error')}}</p>
        @endif
